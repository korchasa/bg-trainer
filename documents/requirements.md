...
- **Limit:** the tallest question in the app is the five-role step-1 frame item «В комнате две книги» (L2). On 375x667 at scale 1.3 its pinned block is 506 px of 594 — 85 %, leaving 88 px, one row of bank tiles. It still works, but that is the edge: a sixth role, or that sentence in a longer wording, would leave nothing to scroll. Two of the sixty-six frame items have five roles and none has more, so the cap is in the data rather than in the layout — a longer frame item has to be checked against this before it is added.
- **Status:** [x]
