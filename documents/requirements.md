...
### 3.10 FR-LESSONS
- **Desc:** 8 lessons aligned with textbook `documents/lessons/lesson-1..8.md`. Each lesson has `id`, `num`, `title`, `modeIds[]`, `available`, `tier`. Lessons 1–7 are fully playable (`available=true`); lesson 8 is a stub (`available=false`, "Скоро"). Tier split for mobile gating (FR-FREEMIUM): L1–L3 = `free`, L4–L8 = `pro`. Web ignores tier.
...
- **Acceptance:**
  - [x] `LESSONS` defined with 8 entries; L1, L2, L3, L4, L5, L6, L7 `available=true`. Evidence: `src/data/lessons.ts`
  ...
  - [x] Data split into per-lesson modules with composition root. Evidence: `src/data/index.ts`, `src/data/lesson1.ts`, `src/data/lesson2.ts`, `src/data/lesson3.ts`, `src/data/lesson4.ts`, `src/data/lesson5.ts`, `src/data/lesson6.ts`, `src/data/lesson7.ts`
