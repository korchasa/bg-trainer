...
- **Acceptance:**
  ...
  - [x] L2 `modeIds` cover L2 grammar topics (27 modes). Evidence: `src/data/lessons.ts` (l2 block)
  - [x] L3 `modeIds` cover L3 grammar topics (32 modes, incl. the question-sentence build; pickFrom/pickOpt for recognition + type for productive recall + timed for automaticity + build/paradigm/match/odd). Evidence: `src/data/lessons.ts` (l3 block)
  - [x] L4 `modeIds` cover L4 grammar topics (28 modes: 6 conjugation paradigms + reflexive paradigm + routine vocab pickFrom/type + time-period + past-time markers + frequency + никога-transform + antonyms + time prepositions + clock pickFrom/type + бях pickOpt/type + ще бъда pick + ще/няма да + combined paradigm + build + hours build + match antonyms + match 1sg↔3sg + odd). Evidence: `src/data/lessons.ts` (l4 block), `src/data/index.ts` (l4_extra category)
  ...
  - [x] Each new L4 mode backed by ≥6 data items. Evidence: `src/data/lesson4.ts` (DATA_L4_CHETA, DATA_L4_UCHA, DATA_L4_KAZVAM, DATA_L4_OPRAVYAM, DATA_L4_PRAVYA, DATA_L4_YAM, DATA_L4_MIYA_SE, DATA_L4_REFL_VOCAB, DATA_L4_TIME_PERIOD, DATA_L4_PAST_TIME, DATA_L4_FREQ, DATA_L4_NEVER, DATA_L4_ANT, DATA_L4_PREP_TIME, DATA_L4_HOURS, DATA_L4_HOURS_TYPE, DATA_L4_BYAH, DATA_L4_BYAH_TYPE, DATA_L4_SHTE_BADA, DATA_L4_SHTE_NEG, DATA_L4_PARADIGM, DATA_L4_BUILD, DATA_L4_HOURS_BUILD, DATA_L4_MATCH_ANT, DATA_L4_MATCH_CONJ, DATA_L4_ODD)
...
- **Acceptance:**
  ...
  - [x] The bank is sorted alphabetically, while `build` and `paradigm` keep shuffling their pool. The bank belongs to the mode — 29–39 words, identical on every question — so the learner recalls the word and only then hunts for it among tiles spread over 940 px; that search practises nothing and takes the attention the recall needs. It was already shuffled once per session rather than per question, so positions were already learnable and sorting gives up nothing. The per-item pools are the opposite case: three to eight words, no search to remove, and the arrangement is itself the answer, which an alphabetical order can coincide with. Cost, counted across all eight banks: alphabetical order files same-stem forms next to each other — 14 of 39 words in L1, 14 of 33 in L2, 12 of 38 in L3, but only 2–6 in L4–L8, so the extra support lands on the steps where scaffolding is highest by design and fades where the ladder means it to. Sorted with `localeCompare(…, "bg")`, since uppercase Cyrillic sorts below lowercase by code point and a plain sort would file every proper noun in a block at the front. Evidence: `scripts/bank.ts`; verified live in L1 — «аз, България, вие, Германия, …»
  ...
  - [x] A word landing in a slot animates in over 300 ms (`.slot-drop`): it drops in from half size and overshoots to 1.16 before settling. The tap and its result are at opposite ends of the screen — the bank at the foot, the sentence pinned at the top — so without motion the tile changes text with nothing for the eye to follow. A first attempt at 0.82→1.0 over 160 ms read as a rendering hiccup rather than as movement. Verified live by sampling the paused animation: scale 0.50 at 0 ms, 0.82 at 60 ms, 1.16 at 180 ms, 1.05 at 240 ms, at rest by 300 ms. Evidence: `src/index.css`, `src/components/engines/FrameEngine.tsx` (tile `key`)
...
- **Acceptance:**
  ...
  - [x] Steps 1–3 share one template line, so the sentence reads as a sentence at every step and the only thing the ladder removes is the role names. A column of labelled rows was the earlier shape; it cost the screen it stood on — the worst item ran 506 px of the 594 px play area at scale 1.3, against 269 px as a line. Evidence: `src/components/engines/FrameEngine.tsx` (`answerArea`)
  - [x] Verified live at every step: L1 (template line with role hints, «У меня есть книга.» → КТО │ ЧТО ДЕЛАЮ │ ЧТО), L2 (roles), L3 (bare slots + per-slot error marking), L5 (free line, alternative order accepted), L7 (typing, capital and final period normalized away, error dialog shows the canonical sentence)
- **Status:** [x]
...
- **Acceptance:**
  ...
  - [x] All 240 non-`paradigm` modes carry ru + uk models. Evidence: `src/data/index.ts`, `deno task test` → "Scanned 250 modes"
...
### 3.36 FR-QUESTION-PINNED
- **Desc:** In every engine whose question is distinct from its answer area, the question is pinned to the top of the play area, so scrolling down to the answers cannot take it out of view. Every engine stacks the screen the same way — a constant instruction with a worked example (FR-TASK-MODEL), then the question, then the answers — and at the larger text sizes (FR-A11Y-TEXT) that stack is taller than a phone screen. Measured on 375x667 at scale 1.3, nine of the fourteen lesson-1 modes overflow: «Страна ↔ язык» by 28 px, «Подбери форму» by 53, «Отрицание» by 118, «На скорость» by 120, «Сделай отрицательным» by 239, «Расскажи о человеке» by 258, «ти → Вие» by 266, «Собери парадигму» by 286, «Сделай вопросом» by 309, and the frame drill by 1063 because its word bank alone is 940 px. Reaching the answers pushed the question off the top and the learner tapped with nothing to answer.
- **What is pinned.** The question, and — in the two sentence-building engines only — the answer area with it. The instruction and the worked example are never pinned: they repeat verbatim on every question of a session and are the tallest part of the stack (120–155 px against the question's 63–126 px), so keeping them on screen would spend it on text already read. In the ten option-style engines the pinned block is the question alone and costs 11–27 % of the play area.
- **Why the sentence engines pin their answer area too.** There the answer is assembled from a word bank that sits below the fold, so the learner taps at the foot of the screen and the words land in slots further up. Pinning the question alone would keep the task in view and hide the work: no way to see which slot fills next, whether the last word landed where it was meant to, or how the sentence reads so far. Pinned together the block costs 398 px of 594 in `build` and 424 px in `frame` at 375x667, scale 1.3 — about two rows of the bank stay visible, which is enough to keep tapping.
- **Supersedes FR-STIMULUS-NEAR-BANK.** That rule put the changing sentence next to the word bank so the two were adjacent, which bought adjacency but not visibility — its own Limit note recorded that L1's 986 px bank still scrolls past a stimulus sitting at its head. Pinning solves the same problem for all engines, not just the two with a bank, so the sentence returns to the top and the adjacency rule is gone along with `scripts/stimulus.ts`. Its one lasting effect is kept: the «Нажми на слова ниже» caption over the bank stays deleted.
- **Mechanism:** `<StickyQuestion>` (`src/components/ui/StickyQuestion.tsx`). Two placement facts, both silent when broken. It must be a **direct child of the engine's scroll root** — a sticky element is confined to its parent's box, so nested inside the centred block it would come unstuck exactly when that block scrolls past, which is when the answers appear and the pinning was supposed to start. And the scroll root carries `p-4 xs:p-6` while a sticky child sticks to the **scrollport's padding edge**, so at `top-0` a padding-tall band above the block stays uncovered and content rides through it; the negative offset cancels that and the block's own padding puts it back inside, both rem-based so they track `--fs-scale`. The block is full-bleed for the same reason an opaque block must be: narrower than the scrollport, it lets answer tiles show through the side gutters.
- **Layout:** a `flex-1` spacer sits above the pinned block as well as below it, so a screen that fits keeps the question group vertically centred as before; when the screen overflows both spacers collapse to nothing. No shadow under the pinned block: on a screen that never scrolls it would draw a divider across the middle of the layout.
- **The opposite end, for one drill.** `paradigm` has the same mechanic — tap a form at the foot, it lands in a row above — but pinning its rows is arithmetically impossible: 125 px of pinned verb plus 429 px of rows is 554 px of the 594 px play area, leaving 40 px for a pool that needs 151–213. So it pins the **pool** instead (`StickyPool`), which keeps the forms reachable from any scroll position. Measured at 375x667, scale 1.3: before, tapping forced the learner to the foot, where the worked-example row — which IS this mode's model, and the reason no example text is printed above it — was entirely off-screen and the next row showed 8 px of its 63. After, the forms are tappable at any scroll position. The worked-example row is pinned with the verb rather than left in the scrolling block: it is this mode's model — the reason no example text is printed above the paradigm — and in the scrolling block it left the screen exactly while the last rows were being filled, which is when a learner most wants to check the pattern. **Limit:** the six rows do not fit whichever end is pinned. With the example row pinned (203 px) and the pool pinned (151–213 px) the band settles at 240 px, which holds three of the five working rows, so one scroll gesture covers a question; the model itself never leaves. Fitting all six would need a row height of about 41 px against today's 63, which is below a comfortable tap target. Compacting the pool tiles (`px-3 py-2 text-sm`) bought 43 px of the gap; the rest is not recoverable.
- **Excluded:** `MatchEngine` and `LiEngine`. In both, the only thing that changes per question **is** the answer area — two columns of tiles, a sentence with tappable gaps — so there is no question to pin above it.
- **Scenario:** On a 375x667 screen at text size «Очень крупный», the learner opens «Сделай вопросом» and scrolls to the fourth option. «Аз съм фотограф» stays at the top of the play area the whole way down, and the next question replaces it in the same place.
- **Acceptance:**
  - [x] Ten engines render their question inside `<StickyQuestion>` as a direct child of the scroll root. Evidence: `deno task test` (`scripts/sticky.ts`)
  - [x] Measured live at 375x667, scale 1.3, scrolled to the foot of the play area: the pinned block sits flush with the top of the scroll area (gap 0 px) in «Расскажи о человеке», «Построй предложение · L1», «Собери парадигму» and «ти → Вие», at 12–27 % of its height
  - [x] The padding-edge leak is closed: before the negative offset, content showed through a 20.8 px band above the pinned question at scale 1.3
  - [x] A screen that fits is unchanged: «Подбери форму» at scale 1.0 has zero overflow and the question group is still centred. Evidence: the `flex-1` spacer above the pinned block
  - [x] Invariant asserted, red first: the scan named all ten engines before the change. Evidence: `scripts/sticky.ts`, `deno task test`
- **Limit:** the tallest question in the app is a step-1 frame item whose role names wrap to three rows — «В комнате две книги» (L2, five roles) and «Его зовут Иван.» (L1, four, one of them «возвратная частица») both reach 269 px of 594 on 375x667 at scale 1.3 — 45 %, leaving 325 px, three rows of bank tiles. What sets the height is the total length of the role names, not the number of slots, so a new frame item is checked against that: the ceiling is roughly three wrapped rows of names, and the earlier column-of-rows shape hit it at 506 px with the same sentence.
- **Status:** [x]
...
### 3.5 FR-REACTION
- **Desc:** After each answer the learner gets a verdict, and a wrong answer reveals the correct one. Which surface carries it depends on whether the engine has a retry dialog. The eight engines built on `useGame` show a green congratulation for a correct answer and leave a wrong one entirely to the FR-RETRY dialog, which names the mistake and gives the correct answer — a bouncing «Мимо!» is born underneath that dialog in the same commit and cannot be read, so it is not produced at all. The four engines that keep their own state (`Build`, `Li`, `Match`, `Paradigm`) have no dialog, so there the red overlay is the whole verdict and stays.
- **Acceptance:**
  - [x] Reaction picked from `OK` / `FAIL` arrays. Evidence: `src/constants.ts:1-2`, `src/utils/shuffle.ts`
  - [x] `useGame` takes only the congratulation (`Reactions = { ok }`) and sets no reaction on a wrong answer. Evidence: `src/hooks/useGame.ts`; verified live in «Подбери форму» — a wrong answer leaves the live region empty and raises the dialog, the next correct answer shows a green «Супер!»
  - [x] Every engine built on `useGame` renders an `<ErrorDialog>`, so dropping the failure message cannot leave a wrong answer with no verdict at all. Asserted red first, twice: reintroducing `pickFail` in the hook, and renaming one engine's dialog, each fail the scan on their own line. Evidence: `scripts/feedback.ts`, `deno task test`
...
- **Acceptance:**
  ...
  - [x] Answers stay character-identical to their fixed option sets across all 44 `pickOpt` modes; no answer duplicates its own decoys. Evidence: verified over all 250 modes / 3940 items — 0 mismatches
