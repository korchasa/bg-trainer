...
### 3.4 FR-ENGINES
- **Desc:** 12 engine types implement distinct interaction patterns. Multiple-choice engines hide L1 hints by default and expose a "Подсказка" reveal button; reveal sets `hinted=true` which is forwarded to `onItemAnswer` and softens mastery effects (see FR-MASTERY).
- **Acceptance:**
...
- **Acceptance:**
  ...
  - [x] Exactly one frame mode per lesson, 8 in total. Evidence: `src/data/index.ts` (`frames` category), `src/data/lessons.ts` (`l1_frame`…`l8_frame`); asserted by `node scripts/check-lesson-lexicon.mjs`
  - [x] Every frame word belongs to its lesson's cumulative lexicon; the lexicon is rebuilt from `LESSONS` + mode data, and frame modes are excluded from the source so none can approve its own vocabulary. Evidence: `scripts/check-lesson-lexicon.mjs`; run output `OK: every frame word comes from its lesson's cumulative lexicon`
  - [x] Bank ≥ 4× the longest sentence of its mode, no duplicates, no two entries differing only by case. Evidence: `scripts/check-lesson-lexicon.mjs`
  - [x] No sentence (or `alt`) uses the same word twice — the bank offers one tile per word, so a repeat would be unfillable and leave the learner unable to submit. Evidence: `scripts/check-lesson-lexicon.mjs`; asserted red by duplicating a word in one L1 sentence → `item "аз не знам аз" repeats "аз" — one tile per word, unfillable`
...
- **Acceptance:**
  ...
  - [x] Step never drops as lessons advance, and all four steps are reached. Evidence: `scripts/check-lesson-lexicon.mjs` (ladder check); run output `step 1,1,2,2,3,3,4,4`
  - [x] `alt` is rejected below step 3, where order and length are fixed and it could never be reached. Evidence: `scripts/check-lesson-lexicon.mjs`
