...
### 3.18a Stimulus placement (FR-STIMULUS-NEAR-BANK)
- **Rule:** in an engine whose answer tiles sit in a word bank at the foot of the screen, the per-question stimulus renders between the answer area and the bank. `BuildEngine` puts the translation after the centred block, directly above the pool — inside that block the `justify-center` slack opens a gap between template and sentence. `FrameEngine` puts it at the head of the bank block, so it exists only when a bank does; at step 4 (`typing`) it stays above the input, where a software keyboard cannot cover it.
- **Why not reading order:** measured on 375×667 at `--fs-scale: 1.3`, the play area overflows by 326 px. Scrolled to the bank, a stimulus above the answer area showed 6 px of its 61; below it, all 61, with the bank fully visible and 153 px of the template still on screen. Nothing writes `scrollTop` except the wheel forwarder in `App.tsx`, so the position survives the question change and the next sentence lands where the learner is already looking.
