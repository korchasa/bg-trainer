...
- **What is pinned and what is not:** the question alone. `TaskPrompt` (instruction + worked example) is constant for the whole session and is the tallest part of the header — 120–155 px against the question's 63–126 px at scale 1.3 — so it stays in the flow and is allowed to scroll away. Pinned this way the block costs 11–27 % of the play area; pinning the instruction with it would roughly double that.
