...
- **Curriculum Scope**: the lessons track one textbook and one level (A0). Before drilling a grammar topic, check `documents/lessons/lesson-1..8.md` for it; if it is not there, say so **in the message that proposes the topic**, not in the plan file afterwards — teaching beyond the course is the user's call, not an implementation detail. Quote the cost as well: a round draws 3 random modes from the lesson, so N new modes in a lesson of M reach `1 - C(M-N,3)/C(M,3)` of rounds. The aorist was built and deleted unused on 2026-08-06 for exactly this reason — 11 modes absent from the textbook, surfacing in 62% of lesson 8's rounds. The same test applies to vocabulary: drills carry no translation, so a verb the learner meets only in a later lesson makes the task unanswerable rather than hard.
