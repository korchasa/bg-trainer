...
export function AnswerBtn({ val, sel, correctVal, onClick, className = "", children }: {
  ...
  let cls =
    "bg-white border-2 border-[#E9E9E9] text-[#111111] hover:border-[#111111] cursor-pointer active:bg-[#111111] active:text-white active:border-[#111111]";
  if (sel !== null) {
    if (val === correctVal) {
      cls = "bg-emerald-500 text-white border-emerald-500 cursor-default";
    } else if (val === sel) {
      cls = "bg-[#E60023] text-white border-[#E60023] cursor-default";
    } else {
  ...
  const ariaLabel = isAnswered
    ? `${val} — ${
      val === correctVal ? t("a11yAnswerCorrect") : val === sel ? t("a11yAnswerWrong") : ""
    }`.trim().replace(/—\s*$/, "")
    : val;
  return (
    <button
